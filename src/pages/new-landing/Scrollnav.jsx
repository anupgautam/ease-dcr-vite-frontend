...
const Scrollnav = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 h-16 lg:h-20 bg-white transition-transform duration-300 content-center ease-in-out z-50 drop-shadow-md ${isVisible ? "transform translate-y-0" : "transform -translate-y-full"
      }`}>
      <div className=" flex font-semibold gap-x-3 lg:gap-x-4 md:gap-x-8 items-center container justify-between">
        ...
        <div className=" flex gap-x-2 lg:gap-x-7 font-bold">
          {
            navItems.map((item, idx) => {
              return (
                <ScrollLink to={item.path} smooth={true} spy={true} offset={-100} duration={800} key={idx} activeClass="active-link">
                  <h1 className=" font-poppins text-[9px] sm:text-[11px] md:text-lg py-2 hover:translate-y-1 duration-500">
                    {item.title}
                  </h1></ScrollLink>
              )
            })
